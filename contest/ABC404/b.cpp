...
int chk(vs tmp, vs t, int n) {
    int res = 0;
    rep(i,0,n) {
        rep(j,0,n) {
            if (tmp[i][j] != t[i][j]) res++;
        }
    }
    return res;
}
...
int main(void) {
    ...
    ans = min(ans, chk(tmp, t, n)+1);

    vs tmp2(n);
    rep(i,0,n) { 
        rep(j,0,n) {
            tmp2[i] += s[n-1-i][n-1-j];
        }
    }
    ans = min(ans, chk(tmp2, t, n)+2);
