...
int main() {
    ...
    for (ll i = 1; ; ++i) {
        ...
        string pal1 = s + rev.substr(1);
        ll num1 = stoll(pal1);
        if (num1 > n) break;
        if (seen.insert(num1).second && check(base(num1, a))) ans += num1;

        string pal2 = s + rev;
        ll num2 = stoll(pal2);
        if (num2 <= n && seen.insert(num2).second && check(base(num2, a))) ans += num2;
    }
